...
func (h *Connection) readOne() bool {
	...
	case "command/reply":
		...
		h.cmd <- resp
	case "api/response":
		if (len(resp.Body) > 5) && string(resp.Body[:2]) == "-E" {
...
func (this *Connection) Bgapi(cmd string) (err error) {
	_, err = this.Send(fmt.Sprintf("bgapi %s", cmd))
	return err
}

func (this *Connection) Api(cmd string) (err error) {
	_, err = this.Send(fmt.Sprintf("api %s", cmd))
	return err
}
